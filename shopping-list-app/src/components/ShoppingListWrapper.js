...
function ShoppingListWrapper({ shoppingListData, selectedList, onSelectList, onDeselectList, currentUser, onUpdateItems, editingListName, onEditListName, onUpdateListName, onUpdateMembers}) {
    return (
        <div className="shopping-list-wrapper">
            {selectedList === null ? (
                shoppingListData.map(list => (
                    <ShoppingListPreview key={list.id} list={list} onSelect={onSelectList} />
                ))
            ) : (
                <div>
